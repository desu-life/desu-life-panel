  ...
  <div class="content">
    <div class="grid">
      <div class="form">
        ...
        <h2>向我们提供你的邮箱</h2>
        <n-form ref="formRef" size="large" :show-label="false" :rules="rules" :model="formModel">
          ...
          <n-form-item v-if="isMobile">
            <n-button secondary block strong type="error" @click="handleClose"> 关闭 </n-button>
          </n-form-item>
        </n-form>
      </div>
      <div class="steps">
        <n-steps :vertical="!isMobile" :current="1">
          <n-step
            title="发送验证邮件"
            :description="
              !isMobile ? '向我们提供你的邮箱，DESU.Life 将会发送一封验证邮件到你的邮箱' : ''
            "
          />
          <n-step
            title="设置密码"
